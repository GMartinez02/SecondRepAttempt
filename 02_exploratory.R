...
#-------------------
variables <- c( "newLocationID"  ,                "Year"  ,
                "Longitude",
                "Latitude"       ,                "fire_normRdNBR_5yDecay",
                "fire_normRdNBR_10yDecay",        "fire_normRdNBR_15yDecay" ,
                "MPB_greenFraction"       ,       "MPB_redFraction"   ,
                "MPB_grayFraction"         ,      "landCover_developedFraction",
                "landCover_agriculturalFraction", "landCover_forestFraction",
                "minTemp_baseFlow"               ,"minTemp_nonBaseFlow",
                "minTemp_yearAvg"                ,"maxTemp_baseFlow",
                "maxTemp_nonBaseFlow"            ,"maxTemp_yearAvg",
                "precip_baseFlow"                ,"precip_nonBaseFlow"  ,
                "precip_yearAvg"                 ,"SWE_baseFlow" ,
                "SWE_nonBaseFlow"                ,"SWE_yearAvg"   ,
                "soil_Afraction"                 ,"soil_Bfraction" ,
                "soil_Cfraction"                 ,"elevation_mean" ,
                "aspect_predominant"             ,"wasteWaterPointSources_count" )
varables <- c(colnames(dataFrame_X_standardized))

for (i in varables){
  plot(dataFrame_X_standardized$i, y_response)
}

plot(dataFrame_X_standardized$elevation_mean, y_response)
